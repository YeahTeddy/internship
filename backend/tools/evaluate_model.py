...
示例：
  ...
  python tools/evaluate_model.py --weights path/to/best.pt --output models/eval_report
    ...
    parser.add_argument("--weights", "-w", type=str, required=True, help="模型权重路径（best.pt）")
    parser.add_argument("--data", "-d", type=str, default=None, help="data.yaml 路径（默认自动查找）")
    parser.add_argument("--split", "-s", type=str, default="val", choices=["train", "val", "test"], help="评估数据集划分")
    parser.add_argument("--conf", type=float, default=0.001, help="置信度阈值（默认: 0.001）")
    parser.add_argument("--iou", type=float, default=0.6, help="NMS IoU 阈值（默认: 0.6）")
    ...
    parser.add_argument("--output", "-o", type=str, default=None, help="评估报告输出目录")
