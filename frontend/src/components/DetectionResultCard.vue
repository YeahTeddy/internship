    ...
    <div class="card-body">
      ...
      <div v-if="isVideo" class="video-result">
        ...
        <div v-else-if="thumbnailFrames.length" class="frames-fallback">
          <p class="fallback-hint">关键帧预览：</p>
          <div class="frames-container">
            <div v-for="(frame, index) in thumbnailFrames" :key="index" class="frame-card" @click="previewVideoFrame(frame)">
              <img :src="`data:image/jpeg;base64,${frame.annotated_image_base64}`" :alt="`帧 ${frame.frame_index}`" />
...
const isVideo = computed(() => {
  return props.result.type === 'video' || (props.result.fps && props.result.processed_frames)
})

/** 视频关键帧缩略图 */
const thumbnailFrames = computed(() => {
  if (!props.result.key_frames) return []
  return props.result.key_frames.filter(f => f.annotated_image_base64).slice(0, 6)
})
...
.frame-card:hover { opacity: 0.8; }
.frame-card img { width: 100%; height: 100px; object-fit: cover; }
